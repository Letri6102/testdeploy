  ...
  return (
    ...
    >
      ...
      <div className="flex-1 flex flex-col items-start justify-start rounded-t-3xl bg-zinc-900 h-full py-6  gap-3 relative">
        {cart && cart?.length > 0 ? (
          <>
            <div className="flex flex-col w-full items-start justify-start gap-3 h-[55%] overflow-y-scroll scrollbar-none px-4">
              {cart &&
                cart?.length > 0 &&
                cart?.map((item, i) => (
            ...
            <div className="bg-zinc-800 rounded-t-[60px] w-full h-[35%] flex flex-col items-center justify-center px-4 py-6 gap-10">
              ...
              <motion.button
                {...buttonClick}
                className="bg-orange-400 w-[80%] px-4 py-3 text-xl text-headingColor font-semibold hover:bg-orange-500 drop-shadow-md rounded-2xl"
    ...
    >
      ...
      <div className="flex items-center justify-start gap-1 w-full">
        ...
        <p className="text-sm flex items-center justify-center gap-1 font-semibold text-red-400 ml-auto">
          <span className="text-red-400" />{"£"}{itemTotal}
        </p>
      </div>
